...
o_gen_quick_res_provision(h,non_vre)=sum(z$(gen_lim(z,non_vre) and
    gen_quick(non_vre) and uc_z(z)),var_res_quick.l(h,z,non_vre));
...
o_variableC=
    ...
    +sum((trans_links(z,z_alias,trans),h),var_trans_flow.l(h,z,z_alias,trans)*
        trans_varom(trans))


Parameter o_nonVREVarC;
o_nonVREVarC=sum((gen_lim(z,non_vre),h),var_gen.L(h,z,non_vre)
    *gen_varom(non_vre))
...
o_capitalC=sum(non_vre,var_new_pcap.L(non_vre)*gen_capex(non_vre))
    ...
    *trans_links_dist(z,z_alias,trans)*trans_line_capex(trans))
    +sum(trans_links(z,z_alias,trans),var_trans_pcap.l(z,z_alias,trans)
    *trans_sub_capex(trans)*2) +sum(g,var_exist_pcap.l(g)*gen_fom(g))
...
o_transCapc=sum(trans_links(z,z_alias,trans),var_trans_pcap.l(z,z_alias,trans)
    *trans_links_dist(z,z_alias,trans)*trans_line_capex(trans))
    +sum(trans_links(z,z_alias,trans),var_trans_pcap.l(z,z_alias,trans)
...
o_capitalstoreC=sum(s,var_new_store_pcap.L(s)*store_pcapex(s)
    +var_new_store_ecap.L(s)*store_ecapex(s))
...
*    (trans_links_dist_bidir(z,z_alias,trans)*trans_loss(trans))));
...
vre_curtail(h,z,vre,r)= vre_gen(h,vre,r)*(var_new_vre_pcap_r.l(z,vre,r)
    +var_exist_vre_pcap_r.l(z,vre,r))-var_vre_gen_r.l(h,z,vre,r);
...
o_grossRet_non_vre(z,non_vre)=sum(h,((o_eprice(z,h)*var_gen.L(h,z,non_vre))
    - (var_gen.L(h,z,non_vre)*gen_varom(non_vre)))) ;


*gross Margins VRE
parameter o_grossRet_vre(z,vre,r);
o_grossRet_vre(z,vre,r)=sum(h,(o_eprice(z,h)*var_vre_gen_r.L(h,z,vre,r))
    - (var_vre_gen_r.L(h,z,vre,r)*gen_varom(vre)))


*gross Margins store
parameter o_grossRet_store(z,s);
o_grossRet_store(z,s)=sum(h,(o_eprice(z,h)*var_store_gen.L(h,z,s))
    - (var_store_gen.L(h,z,s)*store_varom(s)))



scalar o_gen_costs;
o_gen_costs=sum((h,z,non_vre),var_gen.l(h,z,non_vre)*gen_varom(non_vre))
    +sum((vre_lim(vre,z,r),h),var_vre_gen_r.l(h,z,vre,r)*gen_varom(vre));
...
o_vre_cap_z_sum(vre,r)=sum(z,var_new_vre_pcap_r.L(z,vre,r)
    +var_exist_vre_pcap_r.L(z,vre,r));


parameter o_vre_cap_r_sum(z,vre);
o_vre_cap_r_sum(z,vre)=sum(r,var_new_vre_pcap_r.L(z,vre,r)
    +var_exist_vre_pcap_r.L(z,vre,r));
...
*sums up over all hours +regions and gives the generated electricity per
*	renewable type
parameter o_vre_gen_sum_r(vre);
o_vre_gen_sum_r(vre)=sum((h,z,r),var_vre_gen_r.L(h,z,vre,r));


*sums up over all hours +zones and gives the generated electricity per
*	renewable type
...
*sums over all regions and zones and gives the generated electricity per
*	generation type
...
*sums over all hours and gives the electricity generated per zone and
*	store technology
parameter o_store_gen_sum(z,s);
o_store_gen_sum(z,s)=sum(h,var_store_gen.L(h,z,s));


*sums over all hours and zones and gives the electricity generated per zone and
*	store technology
...
price_mean(h)= sum((z,non_vre),(price(z,h)*var_gen.L(h,z,non_vre))
    /sum(vre,var_vre_gen_sum_r(vre)))
...
price_mean(h)=
    (sum((z,non_vre),(price(z,h)*var_gen.L(h,z,non_vre)))+sum((z,vre),
    (price(z,h)*var_vre_gen_sum_r_zone(h,z,vre))))
    /(sum((z,vre),var_vre_gen_sum_r_zone(h,z,vre))+sum((z,non_vre),
    var_gen.L(h,z,non_vre)));
display price_mean;
...
price_mean(h)=
    ...
    +sum((z,s),(price(z,h)*var_store_gen.L(h,z,s))
    /((((sum((z,vre),var_vre_gen_sum_r_zone(h,z,vre))+sum((z,non_vre),
    var_gen.L(h,z,non_vre))+sum((z,s),var_store_gen.L(h,z,s)));
display price_mean;
